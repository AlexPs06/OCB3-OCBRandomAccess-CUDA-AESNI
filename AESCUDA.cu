...
void ExpansionKeys128(const unsigned char *k,unsigned long long klen,  unsigned char keys[11][16] ){
    unsigned char RotWord[4];
    const unsigned char matrizRcon[10]={ 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};
    // keys[0] = (unsigned char) k;
    memcpy(&keys[0], k, 16);
    for(int i = 0; i<10; i++){
        RotWord[0]=keys[i][7];
        RotWord[1]=keys[i][11];
        ...
        for(int j = 0;  j < 4; j++ ){
            RotWord[j] = matrizCajaS[ (int) RotWord[j] ];
        }
        for(int j = 0;  j < 4; j++ ){
            keys[i+1][j*4] =  RotWord[j] ^ keys[i][j*4];
            if(j==0)
                keys[i+1][j*4] = keys[i+1][j*4] ^ matrizRcon[i];
        }
        for(int k = 1;  k < 4; k++ ){
            for(int j = 0;  j < 4; j++){
                keys[i+1][(j*4)+k ] =  keys[i+1][j*4+k-1] ^ keys[i][(j*4)+k];
            }
        }
    }
...
__device__ void addRoundKey(unsigned char* in, unsigned char * keys, int round)
{
	for (int i = 0; i < 16; i++) { 
        in[i] =in[i] ^  keys[(round*16) + i];
    }
}

__device__ void subBytes(unsigned char* in, unsigned char * matrizCajaS)
{
	for (int i = 0; i < 16; i++) { 
        in[i] = matrizCajaS[ (int) in[i]]; 
    }
}



__device__ void shiftRows(unsigned char* in, int *shifttab)
{
    unsigned char h[16];
	memcpy(h, in, 16);
...
__device__ void subBytesMixColumns(unsigned char* in, int * T1, int * T2, int * T3, int * T4){
...
__global__ void Encrypt(aesBlock *m, unsigned long long mlen, unsigned char *keys, unsigned char *result ){
...
void AES128Encrypt(aesBlock *m, unsigned long long mlen, unsigned char *keys, unsigned char *result){
    aesBlock *mCuda;
    unsigned char *keysCuda;
    unsigned char *resultCuda;
    // printf("%x \n",keys[17]);

    int sizeMessage = (mlen/16)*sizeof(class aesBlock);
    int sizeResult = (mlen/16)*sizeof(unsigned char);
    int sizeKeys = 11*16*sizeof(unsigned char);
    ...
    Encrypt<<<nb, nt>>>(mCuda, sizeMessage,keysCuda, resultCuda);
...
int main(int argc, char **argv) {
    ...
    for(int i = 0; i<numBlocks; i++){
        for (int j = 0; j<16;j++){
            aes_block_array[i].block[j]=  m[(i*16)+j];
            // printf("0x%x, ",m[(i*16)+j] );
        }
    }
