...
__device__ void imprimiArregloCuda(int tam,unsigned char *keys ){
    ...
    // }
        printf("%02x ", keys[0+12] & 0xff);
        printf("%02x ", keys[1+12] & 0xff);
        printf("%02x ", keys[2+12] & 0xff);
        printf("%02x ", keys[3+12] & 0xff);
        
       
        printf("%02x ", keys[0+8] & 0xff);
        printf("%02x ", keys[1+8] & 0xff);
        printf("%02x ", keys[2+8] & 0xff);
        printf("%02x ", keys[3+8] & 0xff);
        
        printf("%02x ", keys[0+4] & 0xff);
        printf("%02x ", keys[1+4] & 0xff);
        printf("%02x ", keys[2+4] & 0xff);
        printf("%02x ", keys[3+4] & 0xff);


        printf("%02x ", keys[0] & 0xff);
        printf("%02x ", keys[1] & 0xff);
        printf("%02x ", keys[2] & 0xff);
        printf("%02x ", keys[3] & 0xff);
...
__global__ void OCB128EncryptRandomAcces(aesBlock *m,aesBlock *delta, aesBlock *S,unsigned long long mlen,unsigned long long mlen2, unsigned long long deltalen, unsigned int *keys){
    ...
    if( index<mlen2/16){
        ...
        if(index == (mlen2-16)/16){
            // imprimiArregloCuda(16,(unsigned char *)&delta[0].block[0]);

            for (int i = 0 ; i< 4 ; i++){
                deltaBlock[i]= delta[0].block[i]+index+3;
            }
            // imprimiArregloCuda(16,(unsigned char *)&deltaBlock[0]);
        ...
        //penultimo bloque
            ...
            if(mlen%16==0){
                ...
                //no borrar puede servir para copiar los datos
                // memcpy(aestemp[0].block, m[index].block, 16); //antes estaba mlen
...
__global__ void OCB128EncryptRandomAccesAsociatedData(aesBlock *ad,aesBlock *delta, aesBlock *Ek1, unsigned long long adlen, unsigned long long adlenMultiplo16, unsigned long long deltalen, unsigned int *keys){
    ...
    if( index< (adlenMultiplo16/16)){
        ...
        if(index == ((adlenMultiplo16-16)/16) && adlen%16!=0 ){
            for (int i = 0 ; i< 4 ; i++){
                deltaBlock[i]= delta[0].block[i]+index+1;
            }
            //calculo lamda 5
            OCBAESDelta2Rounds(deltaBlock, keys);
...
__global__ void OCB128DecryptRandomAcces(aesBlock *m,aesBlock *delta, unsigned long long mlen,unsigned long long mlen2, unsigned long long deltalen, unsigned int *keys){
    ...
    if( index<mlen2/16){
        ...
        if(mlen%16==0){
            ...
            XOR_128(m[index].block,deltaBlock);
            // m[index].block[0]= 0x3925841b;
            // m[index].block[1]= 0x02dc09fb; 
            // m[index].block[2]= 0xdc118597; 
            // m[index].block[3]= 0x196a0b32; 
        ...
        if(index == ((mlen2-16)/16) ){
            ...
            shiftRows(m[index].block, shifttab);

            memcpy(aestemp[0].block, m[index].block, mlen%16);
            memcpy(m[index].block, aestemp[0].block, 16);
            
            unsigned char * temp;
            temp = (unsigned char *) m[index].block;
...
void print_hex_string(unsigned int* buf, int len)
{
    int i;
	...
	const unsigned char *k)
{
    ...
    // cout<<"checando    ";
    // for(int i = 0; i<adlenMultiplo16; i++){
    //     imprimiArreglo( 4,asociateData[i].block);
    //     printf("\n---------------------------\n");
    ...
    copyAESBlockToMessage(encrypt, mlen/16, (unsigned int *)&c[0]);
    cout<<"Ciphertext   ";
    print_hex_string((unsigned int *)&c[0], mlen);
    printf("\n---------------------------");
    cout<<endl;
	...
	const unsigned char *k)
{
    ...
    cout<<"Ciphertext   ";
    imprimiArreglo(ceil((clen)/4.0),message);
    printf("\n---------------------------");
    cout<<endl;
...
int main(int argc, char **argv) {
    ...
    crypto_aead_encrypt(c, clen, m, mlen, ad, 0, nonce, npub, k);
    // cout<<"Decrypt"<<endl;
